    ...
    ui(new Ui::MainWindow)
{
    ...
    setWindowFlags(Qt::FramelessWindowHint);  //使窗口的标题栏隐藏
    ...
    mClearVideoAction            = new QAction(QIcon("images/open.png"), QStringLiteral("清空"), this);//右键菜单会出现这些选项
    ...
    connect(mAddVideoAction,     &QAction::triggered, this, &MainWindow::slotActionClick);//当用户触发了与 mAddVideoAction 相关联的动作，triggered 信号就会被发射（emit）。
    //然后，connect 函数将该信号与槽函数 MainWindow::slotActionClick 进行连接，以便在触发动作时执行特定的操作。
    ...
    ui->page_video->setMouseTracking(true);//鼠标跟踪功能，指向就会显示
...
void MainWindow::showOutControlWidget()//显示底部控制控件
{

    mAnimation_ControlWidget->setDuration(800);//设置动画持续时间为 300 毫秒。
...
void MainWindow::grabPicture()
{
    if (mPlayer == nullptr)
    {
         qDebug() << "Video player is not initialized!";
    ...
    QString filename = "E:\\c++yinshipin\\xiangmu\\VideoPlayer\\resources\\screenshot\\screenshot.png";
...
{
    if (event->modifiers() == (Qt::ShiftModifier | Qt::ControlModifier) && event->key() == Qt::Key_S) {
            grabPicture();
        }
}
